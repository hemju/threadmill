...
public class ThreadmillAutoConfiguration {
  ...
  public JobStore threadmillJobStore(ThreadmillProperties properties) {
    ...
    LOG.warn(
        "Threadmill: using explicitly enabled in-memory store — jobs will not survive restart.");
  ...
  @Bean
  @ConditionalOnMissingBean
  public Scheduler threadmillScheduler(
      JobStore store, JobSerializer serializer, LocalWakeBus wakeBus) {
  ...
  @Bean
  @ConditionalOnMissingBean
  public ThreadmillJobRegistry threadmillJobRegistry(
      ApplicationContext context, ThreadmillProperties properties) {
  ...
  public ProcessingNode threadmillProcessingNode(
    ...
    for (var registration : registry.registrations()) {
      LOG.info(
          ...
          registration.maxAttempts() == null
              ? "retry-interceptor default"
   ...
   */
  private static List<String> resolveQueues(
      ProcessingNodeConfig config, ThreadmillJobRegistry registry) {
  ...
  private static String recurringNamespace(
      ThreadmillProperties properties, ApplicationContext context) {

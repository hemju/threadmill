...
- **The SPI.** `JobStore` is the persistence boundary, expressed in operations
  ...
  in `threadmill-test-support` and is held to the same shared contract suite.
...
5. **Scoped values, not `ThreadLocal`.** `JobExecutionContexts.CURRENT` is bound
...
The shared contract tests run as part of every store module's suite; module-local

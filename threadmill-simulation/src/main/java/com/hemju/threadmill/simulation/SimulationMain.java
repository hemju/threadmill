...
public final class SimulationMain {
  ...
  private static RunOutcome runMemory(SimulationConfig config)
  ...
  private static RunOutcome runPostgres(SimulationConfig config)
      throws IOException, InterruptedException {
  ...
  private static Path traceFile(String backend) {
    String name =
        "simulation-" + FILENAME_TIMESTAMP.format(Instant.now()) + "-" + backend + ".jsonl";
    return Path.of("build", "simulation", name);
  }

  record RunOutcome(
      String backend, Path tracePath, SimulationResult result, TraceVerifier.Result verifier) {}
}

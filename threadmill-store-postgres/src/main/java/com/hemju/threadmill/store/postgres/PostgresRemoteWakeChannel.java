...
public final class PostgresRemoteWakeChannel implements RemoteWakeChannel {
  ...
  public PostgresRemoteWakeChannel(
      DataSource publisherDataSource, DataSource listenerDataSource, String channel) {
      ...
      try (Connection conn = listenerDataSource.getConnection();
        ...
        if (live(myGeneration)) {
          ...
          if (consecutiveFailures >= FAILURES_BEFORE_WARN) {
            ...
            LOG.debug(
                "PostgreSQL remote wake listener failed; retrying while polling remains fallback",
                e);
          }
          sleepBeforeRetry();
        }
  ...
  private static String normalizeChannel(String channel) {
    ...
    if (value.getBytes(StandardCharsets.UTF_8).length > 63) {
      throw new IllegalArgumentException(
          "channel must be at most 63 UTF-8 bytes for PostgreSQL LISTEN");
    }
    return value;
  }

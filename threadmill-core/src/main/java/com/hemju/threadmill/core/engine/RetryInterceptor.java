...
public final class RetryInterceptor implements JobInterceptor {
  ...
  public static final String META_INITIAL_BACKOFF_SECONDS =
      "threadmill.retry.initialBackoffSeconds";
  ...
  private final Map<Class<? extends Throwable>, RetryPolicy> byExceptionType =
  ...
  RetryPolicy effectivePolicy(Job job, Throwable cause) {
    ...
    if (attemptsMeta.isPresent() || backoffMeta.isPresent()) {
      try {
        int attempts =
            attemptsMeta.map(s -> Integer.parseInt(s.trim())).orElse(defaultPolicy.maxAttempts());

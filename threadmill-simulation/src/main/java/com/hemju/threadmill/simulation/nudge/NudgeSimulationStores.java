...
final class NudgeSimulationStores {
  ...
  record BackendFixture(ConnectionInfo connectionInfo, AutoCloseable closeAction)
  ...
  static BackendFixture startBackend(Backend backend) {
    return switch (backend) {
      ...
      case REDIS -> {
        ...
        yield new BackendFixture(
            new ConnectionInfo(backend, null, null, null, uri), container::stop);
      }
    };
  }
  ...
  static JobStoreHandle openJobStore(ConnectionInfo connectionInfo) {
    return switch (connectionInfo.backend()) {
      case POSTGRES ->
        new JobStoreHandle(new PostgresJobStore(postgresDataSource(connectionInfo)), null);
      case REDIS -> {
  ...
  private static final class PostgresWorkStore implements WorkStore {
    ...
    private PostgresWorkStore(DataSource dataSource, String runId) {
      try {
        ...
        throw new IllegalStateException(
            "failed to open Postgres nudge simulation work connection", e);
      }
      this.runId = runId;
    }
    ...
    public List<Integer> drain() {
      return transaction(connection -> {
        ...
        try (var statement = connection.prepareStatement(
            "UPDATE nudge_simulation_work SET drained_at = clock_timestamp() "
    ...
    public void close() {
      try {
        ...
        throw new IllegalStateException(
            "failed to close Postgres nudge simulation work connection", e);
      }
    }

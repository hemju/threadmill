...
- **Concurrency pending checks use a partial index.** `threadmill_jobs_concurrency_pending_idx` covers `(concurrency_key, current_state_at)` for `ENQUEUED` / `SCHEDULED` / `AWAITING`; the regression test asserts the plan uses it.
...
- **Redis concurrency state is indexed under the engine slot.** Per-key counters, pending ZSETs, and workflow-root outstanding HASHes live below `{threadmill}:concurrency:{key}:...`; `claim_commit.lua` consults those structures before moving a job to `PROCESSING`, and state-changing scripts keep them in sync with the job hash and active-state indexes.
...
- **Engine concurrency edge case surfaced.** Running `soakMemory` against `mixed-workload` at scale (≥30 s, ≥100 jobs/sec with concurrency keys) consistently strands a small number of jobs in `ENQUEUED` while one job sits in `PROCESSING` forever — the engine's per-state count goes off-by-one and a concurrency key seems to retain a held EXCLUSIVE lock that nothing ever releases. Both signals say `InMemoryJobStore`'s concurrency-group counter handling under fast EXCLUSIVE↔SHARED churn loses an update. The parameterized scenario test passes at 5 s × 80 jobs/sec (below the threshold) and the smoke test passes at 8 s × 50 jobs/sec, so the harness's CI gates are reliable. **Follow-up:** treat as an open bug against `threadmill-store-memory` — turn the failure into a contract test in `AbstractJobStoreContractTest` (it should reproduce against PG / Redis too if real), then fix.

...
- **Build tool:** Gradle (≥ 9.5), multi-module. The wrapper is committed and its distribution SHA-256 is pinned. Gradle dependency locking and checksum verification cover every project, including `buildSrc`; dependency changes update both the relevant `gradle.lockfile` files and `gradle/verification-metadata.xml` in the same contribution. The release OSV gate scans every committed `gradle.lockfile` and npm `package-lock.json`; never point it at the unsupported version catalog or scan historical entries in verification metadata as though they were resolved dependencies. Scanner exceptions in `osv-scanner.toml` are advisory-specific, justified, and time-bounded; never add a package-wide or permanent vulnerability ignore.
...
- `docs/` — public getting-started, Spring quickstart, architecture, handlers, configuration, Redis topology, deduplication, long-running jobs, operations, troubleshooting, migration, and release-checklist docs.
...
- `./gradlew :threadmill-soak:soakEndurance` — production-readiness endurance run: one harness JVM per backend (Postgres + Redis) in parallel; defaults 8h × 50 jobs/s × 3 nodes with node churn every 10m. Pair with `threadmill-soak/docker-compose.endurance.yml` via `-PpostgresUrl` / `-PredisUrl`.
...
- **A job's default id and `createdAt` derive from one clock read.** `Job.Builder.build()` stamps `createdAt` and seeds the default id via `JobId.newId(createdAt.toEpochMilli())`; explicit `.id(...)`/`.createdAt(...)` win. This keeps UUIDv7 id order consistent with `(current_state_at, id)` — the in-key claim-admission order — to within one millisecond. The original two-read shape (id at `Job.builder()`, stamp at `.build()`) diverged unboundedly under thread stalls and produced a false `strictInGroupOrder` abort 11.5h into the first 12h Postgres endurance run: the soak checker orders by the id's millisecond prefix with a 2ms ambiguity band, which is only sound under this invariant. Never split the id and the created stamp back onto separate clock reads.
...
- **One consolidated baseline plus additive migrations.** The entire v1 schema — every table, index, the sharded per-state counter table, and its maintenance trigger — is collapsed into `V1__baseline.sql`, so a fresh database installs in one step. The pre-release index/counter tuning that briefly lived as separate `V2__sharded_job_counts` / `V3__unkeyed_claim_index` / `V4__exclusive_pending_index` / `V5__queue_scoped_pending_index` files was folded back into the baseline before first release (the last moment squashing is free — no deployed database was ever at V1-only). Post-release changes ship as additive migrations and the baseline is never edited again: `V2__cron_task_overrides.sql` adds nullable recurring timeout/attempt overrides (issue #84), `V3__integrity_constraints.sql` rejects invalid scalar state/mode/policy values and negative concurrency bookkeeping, and `V4__cron_state_timing_fingerprint.sql` adds the nullable `threadmill_cron_task_state.timing_fingerprint` column (issue #105 hardening). The `MigrationRunner` and its explicit `SHIPPED_MIGRATIONS` list remain; migrate mode validates the description and checksum of every applied migration before applying pending files.
...
- **Postgres claim decisions are scalar-first — and candidate gathering is queue- and key-driven, never backlog-walking.** Overload stress (16 producers, 400k backlog) exposed three independent O(backlog) costs that decayed claims to ~7/s: the historical (priority, id) pager FOR-UPDATE-walked every concurrency-blocked row; `loadFirstPendingByKey`'s `DISTINCT ON` scanned the whole pending population per pass (no per-group early termination — 130ms×2 at 415k rows); and a first fix's per-key head probes walked foreign-queue ranges because the pending index doesn't lead with queue (1.4s per claim call). `claimReady` now gathers candidates from three bounded FOR UPDATE SKIP LOCKED lanes — unkeyed (`threadmill_jobs_unkeyed_enqueued_idx` partial index), keyed heads per queue-scoped key (`threadmill_jobs_queue_pending_idx` `(queue, concurrency_key, current_state_at, id)` index + recursive-CTE loose key enumeration), and active-hold members by `(concurrency_key, workflow_root_id)` so retried roots reclaim under their own hold — and the earliest-pending admission lookups are per-key LATERAL head probes (EXCLUSIVE probe on the `threadmill_jobs_exclusive_pending_idx` partial index). All three indexes ship in `V1__baseline.sql`. Validated: consumption flat 35–40/s while backlog grew to 200k; producer-free drain ~400/s at 200k (was 6–8/s). Never reintroduce a candidate or admission query whose cost scales with pending rows rather than keys.
...
- **Scoped values, not ThreadLocal.** `EngineScopedValues.CURRENT` is bound around `handler.run(...)`. The binding is inherited by structured-concurrency forks (a `StructuredTaskScope` opened in the handler), but **not** by virtual threads the handler spawns directly via an executor — use `EngineScopedValues.capturing(...)` to carry it across that boundary.
...
- **Wakeups are latency hints, not correctness.** `LocalWakeBus` connects same-JVM schedulers, recurring materialization, and scheduled-job promotion to matching dispatchers so claimable work does not wait out `pollInterval`; Spring wires the shared bus automatically, while manual core wiring must pass the same bus to `Scheduler` and `ProcessingNode`. `RemoteWakeChannel` publishes the same hints across nodes for durable stores: Postgres uses `LISTEN`/`NOTIFY` on `threadmill_wake`, Redis uses Pub/Sub on `{threadmill}:wake`. Both stores expose their channel through the `JobStore.createRemoteWakeChannel` SPI hook (the Redis one shares the store's client without owning it), so the Spring auto-config wires wake channels without instanceof-checking concrete store classes — user-defined store beans get the same wiring as property-configured ones. Wake publish/listen failures are ignored because `pollInterval` remains the correctness fallback.
...
- **Node tags.** `ProcessingNode.Builder.tag(...)` adds a tag to this node. Jobs declare required tags via the metadata key `Dispatcher.REQUIRED_TAGS_META` (comma-separated). After claim, the dispatcher releases mismatched jobs through `JobRunner.releaseWithoutRunning` (see the engine note above). Dispatcher-side filtering; store-side filtering can be added later if profiling demands it.
...
| Redis claim walks the queue backlog or every registered key (O(backlog) HGETALL + per-key lock cycles per pass; <1 claim/s at 1.7M backlog, producer inserts starved into lock timeouts) | `RedisJobStoreRegressionTest.claimCandidateGatheringIsBoundedByKeysNotBacklogDepth`, `claimCandidateGatheringUsesABoundedRotatingKeyScan`, and `rotatingKeyScanEventuallyReachesClaimableWorkBeyondTheFirstPage` + the extended `assertRedisIndexesConsistent` covering `queue_unkeyed` / `queue_keys` / `pending_root` |
...
| Dashboard base path diverges between controller property and DashboardOptions; static UI mount served unauthenticated | `ThreadmillDashboardUiMountTest.customOptionsBeanWithDivergentBasePathFailsFast` + `customOptionsBeanMatchingThePropertyStarts` + `ThreadmillDashboardSecurityIntegrationTest.staticUiMountRequiresAuthenticationByDefault` |
| Dashboard security chain silently skipped when the host relies on the security starter auto-configuration (missing after-edge) | `ThreadmillDashboardSecurityStarterAutoConfigTest.dashboardChainIsCreatedWhenTheHostReliesOnSecurityStarterAutoConfiguration` + `documentedSessionAndCsrfBehaviorApplies` |
| Handler fan-out via plain virtual-thread executors silently loses the scoped-value context; Redis store lacks the documented `createRemoteWakeChannel` override | `EngineScopedValuesTest.plainVirtualThreadExecutorDoesNotInheritTheBinding` + `capturingRebindsTheContextAcrossAnExecutorBoundary` + `RedisRemoteWakeChannelTest.storeCreatedChannelDeliversWakesAndLeavesTheStoreUsable` |
...
| Soak exclusivity judged from post-commit interceptor lock events false-aborts on a legal µs per-key handoff (first real 8h endurance run) | `InvariantViolationTest.exclusivityHeldIgnoresInterceptorLockEventHandoffOverlap` + `exclusivityHeldFiresWhenHandlersExecuteConcurrently` + `strictInGroupOrderPassesWhenTheEarlierExclusiveAlreadyExecuted` + `strictInGroupOrderDoesNotLetALateRetryExcuseARealLeapfrog` |
| Streaming soak invariant misses its red path (silently disabled checker) or unbounded violation recording exhausts memory | `InvariantViolationTest` (red path per invariant + mid-run-vs-final semantics + violation cap) + `LiveInvariantVerifierTest.firstDefiniteViolationFiresTheCallbackExactlyOnce` + `aThrowingCheckIsQuarantinedAndReportedWithoutMaskingOthers` |
| Node churn loses work or churns the last node; endurance orchestrator drops a child's verdict | `NodeChurnSmokeTest.churnedRunDrainsCompletelyAndRecordsTheChurnCycles` + `churnWithASingleNodeIsRejectedAtConfigTime` + `EnduranceOrchestratorSmokeTest.parallelChildrenProduceCollatedSummaryAndPerBackendArtifacts` |
| External-Redis soak fixture flushes a shared instance instead of resetting the Threadmill namespace | `RedisExternalUrlFixtureTest.externalUrlRunResetsOnlyTheThreadmillNamespace` |
| Quarantine of an unreadable keyed job wedges its concurrency KEY (dangling Redis pending member; unreleased workflow hold on both backends) | `RedisJobStoreRegressionTest.quarantinedKeyedWorkflowMemberReleasesItsPendingEntryAndHoldShare` + `PostgresJobStoreRegressionTest.quarantinedKeyedWorkflowMemberReleasesTheWorkflowHold` |
| Redis soft-delete of a never-claimed keyed job fabricates a hold entry and frees another root's in-flight slot | `RedisJobStoreRegressionTest.softDeleteOfANeverClaimedKeyedJobDoesNotReleaseAnotherRootsHold` |
| Workflow reconciliation's fixed newest-first window permanently shadows stranded children at fan-out scale | `WorkflowReconciliationTest.rescuesAStrandedChildBeyondTheFirstSearchWindow` |
| Rolling deploys burn retry attempts (shutdown interrupt classified as handler failure) | `ProcessingNodeTest.shutdownInterruptedJobIsRescheduledWithoutBurningAnAttempt` + `RetryInterceptorTest.shutdownInterruptRescheduledImmediatelyWithoutConsumingAnAttempt` |
| Crash between the FAILED save and the reschedule save strands a job with unspent retry budget forever | `RetryInterceptorTest.recoveryScanReschedulesAStrandedFailedJobWithBudget` + `recoveryScanLeavesFinalFailedJobsAlone` + `recoveryScanLeavesYoungFailedJobsToTheLiveHook` |
| Multi-producer stress false-flags in-key order (enqueue emission order diverges from creation order) | `InvariantViolationTest.strictInGroupOrderUsesCreationOrderNotEmissionOrder` + `strictInGroupOrderStillFlagsARealLeapfrogWithUuidIds` + `strictInGroupOrderExcusesTheSameInstantAmbiguityBand` |
| Single-row per-state counters convoy every concurrent Postgres writer (13 jobs/s under 16 producers) | `PostgresJobStoreRegressionTest.emittedMigrationSqlAppliesToACleanSchema` asserts the baseline installs 16 counter shards per state; the load regression is the soak stress shape (`-Pproducers=16`) |
| Job id and `createdAt` from two clock reads let UUIDv7 order contradict the `(current_state_at, id)` admission order (false `strictInGroupOrder` endurance abort) | `JobTest.defaultIdTimePrefixMatchesCreatedAtFromASingleClockRead` + `explicitIdAndCreatedAtAreNotOverriddenByTheDerivedDefaults` + `millisSeededIdKeepsUuidV7VersionAndVariantBits` |
| Dispatcher release of a claimed-but-unrun job attempts illegal PROCESSING→SCHEDULED (always throws; churn leaves jobs to orphan reclaim, burning attempts and holding concurrency slots) | `ProcessingNodeTest.tagMismatchReleaseRoutesThroughTheFailurePathWithoutBurningAnAttempt` + `interruptedMidBatchDispatchReleasesRemainingClaimedJobsForRedelivery` |
| `@Job(timeout)` / `@Job(maxRetries)` silently ignored for recurring jobs — CronTask cannot carry them, materialized instances run under the engine defaults (issue #84) | `SchedulingTest.recurringInstancesCarryTheTaskTimeoutAsPerJobTimeoutMetadata` + `recurringInstancesWithoutATaskTimeoutCarryNoTimeoutMetadata` + `AbstractJobStoreContractTest.cronTaskTimeoutRoundTripsAndClearsOnReUpsert` + `ThreadmillAutoConfigurationTest.recurringHandlerTimeoutLandsOnTheRegisteredCronTask` + `DashboardApiServiceTest.updateRecurringPreservesTheTaskTimeout` + `manualTriggerCarriesTheTaskTimeoutAsPerJobTimeoutMetadata` |
| `@Job(maxRetries = 0)` silently becomes the engine default of five attempts; the value feeds attempts under a retries name; the stamped default shadows per-exception-type retry policies for every Spring-enqueued job (issue #104) | `ThreadmillAutoConfigurationTest.nonPositiveMaxAttemptsFailsStartupInsteadOfBecomingTheDefault` + `JobSchedulerRetryMetadataTest.explicitMaxAttemptsIsStampedAsPerJobRetryMetadata` + `defaultRetryBudgetLeavesRetryMetadataUnstamped` + `RetryInterceptorTest.singleAttemptBudgetMakesTheFirstFailureTerminalWithoutReschedule` |
| Startup re-registration resets `next_run_at`, so `CATCH_UP` never recovers restart-missed firings; interval phase re-based on every restart (issue #105) | `SchedulingTest.restartReRegistrationPreservesOverdueNextRunSoCatchUpRecoversMissedFires` + `restartReRegistrationWithDropCollapsesMissedFiresIntoASingleRun` + `reRegistrationWithAChangedTriggerRecomputesNextRunFromNow` + `reRegistrationWithAChangedZoneRecomputesNextRun` + `unchangedReRegistrationPreservesTheIntervalPhase` + `reEnablingATaskRestartsTimingInsteadOfCatchingUpTheDisabledPeriod` + `CronExpressionTest.equalityIsValueBasedOnTheSourceExpression` + `AbstractJobStoreContractTest.cronTaskTriggerRoundTripsToEqualValues` |
| Crash between the separate cron-task and schedule-state writes pairs a new trigger with old timing that the retry then preserves forever; a store dropping the fingerprint silently reverts restart recovery | `SchedulingTest.stateWriteFailureDuringTriggerEditCannotPairNewTriggerWithOldTiming` + `AbstractJobStoreContractTest.cronTaskStateTimingFingerprintRoundTrips` |
| `DROP` recovery re-bases the schedule on the recovery wall-clock (interval phase drifts by the downtime; `cronFireTime()` lies about the firing it represents); an interval's documented-ignored zone counts as a schedule edit across nodes with different system zones | `SchedulingTest.restartReRegistrationWithDropCollapsesMissedFiresIntoASingleRun` (exact nominal-fire and phase assertions) + `differentSystemZonesDoNotResetAnIntervalSchedule` |
...
- **Add-a-store skill** (informal): create a `threadmill-store-X` module, implement `JobStore`, and add an integration test class extending `AbstractJobStoreContractTest`. Pass every contract test (currently 78) before adding any backend-specific tests.
...
- **Run the load soak harness**: `./gradlew :threadmill-soak:soakMemory` / `soakPostgres` / `soakRedis` / `soakAll` — operator-driven, per-backend, per-scenario stress with `-P` knobs (`scenario`, `duration`, `jobsPerSecond`, `producers`, `workerCount`, `nodes`, `nodeChurn`, `outputDir`, `runId`, `failFast`, `force`, `postgresUrl`, `redisTopology`, `redisUrl`, `progressInterval`). Eight scenarios (`mixed-workload` default; `rw-lock-stress`, `weighted-queues`, `retry-storm`, `long-running`, `pause-resume`, `bulk-enqueue`, `crash-recover`). Each run writes a self-contained directory under `build/soak/<scenario>-<backend>-<timestamp>/` with `summary.json` + `summary.md` + `progress.json` + `trace.jsonl` + `metrics.jsonl` + `latencies.jsonl` + `lock-events.jsonl` + `invariants.json` + `config.json` + log captures. `summary.json` validates against a JSON Schema 2020-12 file shipped at `threadmill-soak/src/main/resources/com/hemju/threadmill/soak/harness/summary.schema.json`. See `threadmill-soak/README.md` for the AI-drop-in workflow.
- **Run the endurance sign-off**: `docker compose -f threadmill-soak/docker-compose.endurance.yml up -d`, then `./gradlew :threadmill-soak:soakEndurance "-PpostgresUrl=jdbc:postgresql://localhost:54320/threadmill?user=threadmill&password=threadmill" -PredisUrl=redis://localhost:63790` — Postgres + Redis harness JVMs in parallel for 8h (all harness knobs apply; do a 90s pre-flight first). Read `endurance-summary.md`, then drop the whole run directory into an AI conversation for anything deeper.
...
- **Reproducible production-grade benchmarks** separate from the soak suite.

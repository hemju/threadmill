...
public record ProcessingNodeConfig(
  ...
  public ProcessingNodeConfig {
    ...
    if (defaultMaxAttempts <= 0)
      throw new IllegalArgumentException("defaultMaxAttempts must be positive");
    if (logMaxRatePerSecond <= 0)
      throw new IllegalArgumentException("logMaxRatePerSecond must be positive");
    ...
    if (!maintenanceLeaseDuration.minus(claimHeartbeat).isPositive()) {
      throw new IllegalArgumentException(
          "maintenanceLeaseDuration must be greater than claimHeartbeat");
    }
    ...
    if (!nodeHeartbeatRetention.minus(heartbeatTimeout).isPositive()) {
      throw new IllegalArgumentException(
          "nodeHeartbeatRetention must be greater than heartbeatTimeout");
    }

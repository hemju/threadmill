...
final class NudgePumpSmokeTest {
    ...
    @Test
    void nudgeDrivenPumpDrainsItsOutboxAndBothNudgeInvariantsPass(@TempDir Path tempDir) throws Exception {
        ...
        try {
            ...
            assertThat(trace)
                    .as("pump runs must be nudge-triggered — the backstop schedule cannot fire in a run this short")
    ...
    private static void clearSoakSystemProps() {
        for (String k : new String[] {
            "scenario", "duration", "jobsPerSecond", "workerCount", "nodes", "outputDir", "runId", "failFast", "force"
        }) {

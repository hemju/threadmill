...
class RetryInterceptorTest {
  ...
  void shutdownInterruptRescheduledImmediatelyWithoutConsumingAnAttempt() {
    ...
    interceptor.onProcessingFailed(
        failed,
        null,
        new InterruptedException("node closing"),
    ...
    Thread registrar = Thread.ofVirtual().unstarted(() -> {
      try {
        ...
        for (int i = 0; i < 2_000; i++) {
          interceptor.policyFor(
              RuntimeException.class, RetryPolicy.of(2, Duration.ofMillis(i + 1)));
          interceptor.policyFor(
              IllegalStateException.class, RetryPolicy.of(3, Duration.ofMillis(i + 1)));
        }
      } catch (Throwable t) {
